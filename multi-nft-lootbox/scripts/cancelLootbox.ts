...
export async function run(provider: NetworkProvider, args: string[]) {
    ...
    await lootbox.sendCancel(provider.sender(), toNano('0.05'));

    ui.write('Waiting for lootbox to be canceled');

    let attempt = 1;
    while ((await lootbox.getChancesData()).chancesLength > 0) {
    ...
    ui.write(`Lootbox was successfully canceled`);
}
